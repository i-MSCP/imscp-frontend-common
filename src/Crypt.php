 ...
 * Class Crypt
 *
 * Provides functions for passwords hashing, verification and data encryption.
 *
...
class Crypt
{
     ...
     * @return string
     */
    static public function md5(string $password, string $salt = NULL): string
    {
        if ($salt !== NULL) {
            if (strlen($salt) < 8) {
     ...
     * @return string
     */
    static public function randomStr(int $length, string $charList = self::BASE64): string
    {
     ...
     * @return string
     */
    static public function sha256(string $password, int $rounds = 5000, string $salt = NULL): string
    {
        if ($salt !== NULL) {
            if (strlen($salt) < 16) {
     ...
     * @return string
     */
    static public function sha512(string $password, int $rounds = 5000, string $salt = NULL): string
    {
        if ($salt !== NULL) {
            if (strlen($salt) < 16) {
                throw new \InvalidArgumentException('The salt must be a string at least 16 bytes long');
     ...
     * @return string
     */
    static public function htpasswd(string $password, int $cost = 10, string $salt = NULL, string $format = 'md5'): string
    {
        switch ($format) {
            case 'bcrypt':
            ...
            case 'crypt':
                if ($salt !== NULL) {
                    if (!is_string($salt) || strlen($salt) != 2) {
            ...
            default:
                throw new \InvalidArgumentException(sprintf('Unknown %s format. Supported formats are: bcrypt, crypt, md5 and sha1', $format));
        }
    }
     ...
     * @return string
     */
    static public function bcrypt(string $password, int $cost = 10, string $salt = NULL): string
    {
        $cost = (int)$cost;
        if ($cost < 4 || $cost > 31) {
     ...
     * @return string
     */
    static public function apr1MD5(string $password, string $salt = NULL): string
    {
        if ($salt !== NULL) {
     ...
     * @return string
     */
    static protected function toAlphabet64(string $string): string
    {
     ...
     * @return bool
     */
    static public function hashEqual(string $knownString, string $userString): bool
    {
     ...
     */
    static function encryptBlowfishCBC(string $key, string $iv, string $data): string
    {
     ...
     */
    static protected function encrypt(string $algorithm, string $key, string $iv, string $data): string
    {
     ...
     * @return string
     */
    static function decryptBlowfishCBC(string $key, string $iv, string $data): string
    {
     ...
     * @return string
     */
    static protected function decrypt(string $algorithm, string $key, string $iv, string $data): string
    {
        if (!extension_loaded('openssl')) {
            throw new \RuntimeException('OpenSSL extension is not available');
        }
     ...
     * @return string A base64 encoded string representing encrypted data
     */
    static function encryptRijndaelCBC(string $key, string $iv, string $data): string
    {
        switch (strlen($key)) {
            case 16:
     ...
     * @return string
     */
    static function decryptRijndaelCBC(string $key, string $iv, $data): string
    {
        switch (strlen($key)) {
            case 16:
                $algorithm = 'AES-128-CBC';
